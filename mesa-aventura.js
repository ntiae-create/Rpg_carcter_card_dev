/* ==========================================
   MESA ONLINE — RPG
   SISTEMA DE AVENTURA
   MAPA • DUNGEON • COMBATE • BOSS • EVENTOS
========================================== */
...
(() => {
    ...
    function inicializar() {
        ...
        console.log("🗺️ MesaAventura inicializada.");
    }
    ...
    function receberAventura(evento) {
        ...
        switch (tipo) {
            case AVENTURA_CONFIG.tipos.MAPA:
                renderizarMapa(dados.dados || {});
                break;

            case AVENTURA_CONFIG.tipos.DUNGEON:
                renderizarDungeon(dados.dados || {});
                break;

            default:
                renderizarEvento(dados);
                break;
        }
    }
    ...
    function abrirMapa(dados = {}) {
        const estado = obterEstado();

        if (estado) {
            estado.aventura = estado.aventura || {};
            estado.aventura.tipo = "mapa";
            estado.aventura.dados = dados;
        }
    ...
    function renderizarMapa(dados = {}) {
        ...
        MesaUI.screen.classList.add("aventura-ativa");
        ...
        MesaUI.screenContent.innerHTML = `
            <section class="aventura-view mapa-view">
                ...
                <footer class="aventura-footer">
                    <span>
                        👥 ${contarJogadoresAtivos()} jogadores
                    </span>
    ...
    function selecionarPontoMapa(local) {
        const nomes = {
            inicio: "Ponto inicial",
            exploracao: "Área de exploração",
            objetivo: "Objetivo da aventura"
        };

        const nome =
            nomes[local] || "Local desconhecido";
    ...
    function abrirDungeon(dados = {}) {
        const estado = obterEstado();

        if (estado) {
            estado.aventura = estado.aventura || {};
            estado.aventura.tipo = "dungeon";
            estado.aventura.dados = dados;
        }

        renderizarDungeon(dados);
    ...
    function renderizarDungeon(dados = {}) {
        atualizarReferencias();
    ...
    function executarAcaoDungeon(acao) {
        const estado = obterEstado();
    ...
    function receberBatalha(evento) {
        const estado =
        ...
        inicializarBatalha(estado);
    }


    function inicializarBatalha(estado) {
        atualizarReferencias();
        ...
        if (tipo === "boss") {
            renderizarBoss(estado);
            return;
        }

        renderizarCombate(estado);
    ...
    function renderizarCombate(estado) {
        atualizarReferencias();
    ...
    function restaurarBatalha(estado) {
        atualizarReferencias();
        ...
        if (
            ...
            renderizarTelaAventuraAtual(estado);
            return;
        }
    ...
    function receberBatalhaFinalizada(evento) {
        const estado =
            evento?.detail?.estado ||
            obterEstado();
    ...
    function renderizarBoss(estado) {
        atualizarReferencias();
            ...
            <section class="aventura-view boss-view">
                ...
                <div class="boss-party">
                    ...
                    <div class="boss-players">
                        ${renderizarMiniJogadores(estado)}
                    </div>
    ...
    function renderizarMiniJogadores(estado) {
        const jogadores =
            estado.jogadores || [];

        return jogadores
            .slice(0, AVENTURA_CONFIG.maxJogadores)
            .map((jogador, indice) => {
    ...
    function renderizarTelaAventuraAtual(estado) {
        if (!estado) return;

        const aventura =
            estado.aventura || {};

        switch (aventura.tipo) {
    ...
    function receberMudancaModo(evento) {
        const modo =
            evento?.detail?.modo;
    ...
    window.MesaAventura = {
        ...
        mapa: {
            abrir: abrirMapa,
            renderizar: renderizarMapa,
            selecionarPonto: selecionarPontoMapa
        },

        /* DUNGEON */
        dungeon: {
            abrir: abrirDungeon,
            renderizar: renderizarDungeon,
            explorar: () =>
                executarAcaoDungeon("explorar"),
            avancar: avancarSalaDungeon,
            voltar: voltarSalaDungeon
        },

        /* COMBATE */
        combate: {
            iniciar: inicializarBatalha,
            renderizar: renderizarCombate,
            restaurar: restaurarBatalha,
            resultado: mostrarResultadoCombate
        },

        /* BOSS */
        boss: {
            abrir: abrirBoss,
            renderizar: renderizarBoss
        },

        /* EVENTOS */
        eventos: {
            abrir: abrirEvento,
            renderizar: renderizarEvento
        },

        /* GERAL */
        atualizar: renderizarTelaAventuraAtual,
        notificacao: mostrarNotificacaoAventura
    };
