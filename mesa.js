...
function registrarEventos() {
    ...
    */
    ...
    /*
     ===========================================================
     CLIQUES — DELEGAÇÃO À PROVA DE SUPABASE / REALTIME
     ===========================================================

     Registrado UMA vez no document (capture).
     Não morre quando o Realtime atualiza cards ou innerHTML.
    */

    if (
        document.body &&
        document.body.dataset.mesaClickDelegado ===
            "true"
    ) {
            ...
            try {
                ...
                */
                ...
                if (
                    ...
                    if (
                        event.target.closest(
                            "button, a, input, select, textarea, [data-mesa-action]"
                        )
                    ) {

                        /*
                         deixa botões internos / data-mesa-action
                         serem tratados no fluxo abaixo ou no button-mesa
                        */
                ...
                /*
                 Botões data-mesa-action
                 (volta, mesa, fechar, cte, etc.)
                */
                ...
                */
                ...
                if (
                    mapPoint &&
                    window.MesaAventura &&
                    typeof window.MesaAventura.selecionarPontoMapa ===
                        "function"
                ) {

                    window.MesaAventura.selecionarPontoMapa(
                        mapPoint.dataset.mapPoint
                    );
                ...
                if (
                    dungeonBtn &&
                    window.MesaAventura &&
                    typeof window.MesaAventura.executarAcaoDungeon ===
                        "function"
                ) {

                    window.MesaAventura.executarAcaoDungeon(
                        dungeonBtn.dataset.dungeonAction
                    );
                ...
                console.error(
                    "[Mesa] Erro no handler de clique (UI não deve parar):",
                    erroClick
                );
