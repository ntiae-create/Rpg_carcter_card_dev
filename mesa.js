...
function registrarEventos() {
    // eventos principais ficam nos módulos auxiliares
}
...
function inicializarSubmodulos() {
    ...
    if (
        window.MesaAventura &&
        typeof window.MesaAventura.inicializar === "function"
    ) {
        try {
            window.MesaAventura.inicializar();
        } catch (erro) {
            console.warn("[Mesa] Erro ao inicializar MesaAventura:", erro);
        }
    }
}
...
function inicializarMesa() {
    ...
    // segunda tentativa (auth/supabase podem atrasar)
    setTimeout(function () {
        carregarContextoUsuario();
        sincronizarRealtimeCampanha();
    }, 800);
